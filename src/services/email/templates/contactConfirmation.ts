...
export function generateContactConfirmationEmail(data: ContactConfirmationData): { html: string; text: string; subject: string } {
  const subject = `Message Received — ${BUSINESS_CONFIG.name}`;
...
<head>
  ...
  <style>
    ...
    .box { background: #f9f9f9; border-left: 3px solid #111111; padding: 15px; margin: 20px 0; font-size: 13px; color: #555555; }
...
<body>
  <div class="container">
    ...
    <p>A member of our architectural glass team will review your inquiry and get back to you promptly.</p>
  ...
  const text = `
Message Received — ${BUSINESS_CONFIG.name}

Dear ${data.name},

Thank you for reaching out to Complete Glass Innovations. We have received your message:

"${data.message}"

A member of our architectural glass team will review your inquiry and get back to you promptly.
